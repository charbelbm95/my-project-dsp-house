...
if 'prediction_time' not in columns:
    cursor.execute("ALTER TABLE predictions ADD COLUMN prediction_time TIMESTAMP")
if 'source' not in columns:
    cursor.execute("ALTER TABLE predictions ADD COLUMN source TEXT")
conn.commit()
...
async def predict_csv(file: UploadFile = File(...), prediction_source: str = Form(...)):
    try:
        ...
        for index, row in df.iterrows():
            cursor.execute("INSERT INTO predictions (inputs, prediction, prediction_time, source) VALUES (?, ?, ?, ?)", 
                           (str(row[:-1].tolist()), row['Net_Hourly_Electrical_Energy_Output'], prediction_time, prediction_source))
        conn.commit()
